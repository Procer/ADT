                        ...
                        {loading && logs.length === 0 ? (
                            ...
                            logs.map((log, idx) => (
                                <tr key={idx} style={{ borderBottom: '1px solid var(--glass-border)', fontSize: '0.85rem' }}>
                                    ...
                                    <td style={{ padding: '1rem' }}>
                                        <div style={{ fontSize: '0.7rem', opacity: 0.6, maxWidth: '400px', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                            {log.context ? (
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
                                                    {log.context.state && <span>Estado: <b>{log.context.state}</b></span>}
                                                    {log.context.error && <span style={{ color: '#fca5a5' }}>Err: {log.context.error}</span>}
                                                    <span style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                                                        <Smartphone size={10} /> {log.context.userAgent?.split(') ')[1]?.split(' ')[0] || 'Unknown Device'}
                                                    </span>
                                                </div>
                                            ) : '-'}
                                        </div>
