...
export class AppLog {
    ...
    @Column({ name: 'contexto', type: 'nvarchar', length: 100 })
    contexto: string; // Ej: SMTP_SERVICE, BILLING, AI_COPILOT

    @Column({ name: 'level', type: 'nvarchar', length: 20, default: LogLevel.ERROR })
    level: LogLevel;
    ...
    @Column({ name: 'user_id', type: 'nvarchar', length: 255, nullable: true })
    userId: string | null; // Si el error fue de un usuario logueado

    @Column({ name: 'tenant_id', type: 'nvarchar', length: 255, nullable: true })
