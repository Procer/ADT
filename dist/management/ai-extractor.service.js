...
let AiExtractorService = AiExtractorService_1 = class AiExtractorService {
    dataSource;
    logger = new common_1.Logger(AiExtractorService_1.name);
    genAI;
    ...
    async extractTripData(text, apiKey) {
        const clientApiKey = apiKey;
        if (!clientApiKey)
            throw new Error('Servicio de IA no configurado para esta empresa');
        const genAI = new generative_ai_1.GoogleGenerativeAI(clientApiKey);
        const modelsToTry = [
            "gemini-2.5-flash",
            "gemini-2.5-pro",
            "gemini-2.0-flash",
    ...
    async callWithRetry(fn, retries = 3, delay = 2000) {
        try {
            return await fn();
        }
        catch (error) {
    ...
    async processFinanceCopilot(userInput, context, apiKey) {
        const clientApiKey = apiKey;
        if (!clientApiKey)
            throw new Error('Servicio de IA no configurado para esta empresa');
        const genAI = new generative_ai_1.GoogleGenerativeAI(clientApiKey);
    ...
    async executeReadOnlyQuery(sql, tenantId) {
